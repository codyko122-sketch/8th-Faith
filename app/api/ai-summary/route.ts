...
- tips 는 이번 여행·이 피부타입·이 수질 특성·(있다면) 알러지 성분에 실제로 도움이 될 실행 가능한 케어 팁 2~3개. 각 팁은 이모지로 시작하는 한 문장.
...
function demoSummary(input: AiSummaryInput): AiSummaryResult {
  ...
  if (lang === "jp") {
    ...
    return {
      summary: `${env}${input.placeLabel}で${skin.skinTypeForRec}なら、${advice[skin.skinTypeForRec] ?? "基本の保湿と紫外線対策を心がけましょう。"} ${waterQuality.note}`,
      tips,
      demo: true,
    };
  }
  if (lang === "en") {
    ...
    return {
      summary: `With ${env} conditions in ${input.placeLabel} and ${skin.skinTypeForRec}, ${advice[skin.skinTypeForRec] ?? "stick to basic hydration and sun protection."} ${waterQuality.note}`,
      tips,
      demo: true,
    };
  }
